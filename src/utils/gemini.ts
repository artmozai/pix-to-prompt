...
export const getGeminiResponse = async (imageData: any) => {
  ...
  const systemPrompt = `You are a visual prompt generator for AI image generation. Analyze images and output precise, technical descriptions focusing on:
...
Focus on reproducible visual elements that enable accurate image recreation.`;
  ...
  const prompt = systemPrompt + critical;
